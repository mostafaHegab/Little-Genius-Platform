...
function listenToCloudData(target = "home") {
...
function fallbackToLocalStorage(target = "home") {
...
async function fetchActiveUsers() {
	...
	try {
		if (isCloudMode && db) {
			...
			console.log("Active users fetched:", usersSnapshot.size);
			...
			usersListDiv.innerHTML = "<p class='text-sm text-gray-500'>تعذر جلب المستخدمين، يرجى التحقق من الاتصال بالإنترنت.</p>";
		}
	} catch (err) {
